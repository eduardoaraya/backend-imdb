...
class RegisterController {
  async execute(req, res) {
    try {
      ...
      const checkIfEmailexist = await Admin.findOne({ where: { email } });
      ...
      const token = new AuthenticateService(Types.Admin).genToken(user);
      return res.status(201).json({
        message: "Success creating administrator " + user.email,
        token
      });
    } catch (error) {
      console.log(error);
